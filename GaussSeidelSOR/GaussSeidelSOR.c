...
int main(int argc, char ** argv) {
    ...
    if (argc!=5) {
        ...
        global[0]=atoi(argv[1]);    //1024x1024
        global[1]=atoi(argv[2]);
        grid[0]=atoi(argv[3]);      //8x8
        grid[1]=atoi(argv[4]);
    }
    ...
    for (i=0;i<2;i++) {
        if (global[i]%grid[i]==0) {
            local[i]=global[i]/grid[i];     //128x128
	...
	*/
    ...
    if(north != MPI_PROC_NULL) i_min = 1;   //Has up-neighbor
    else i_min = 2;
    if(south != MPI_PROC_NULL) i_max = local[0];    //Has down-neighbor
    ...
    for (t=0;t<T;t++) {
        ...
        MPI_Waitall(req_counter, request, status);
    ...
    if (rank==0) {
        #ifdef TEST_CONV
        printf("GaussSeidelSOR X %d Y %d Px %d Py %d Iter %d ComputationTime %lf ConvergenceTime %lf TotalTime %lf midpoint %lf\n",global[0],global[1],grid[0],grid[1],t,comp_time,conv_time,total_time,U[global[0]/2][global[1]/2]);
        #endif
