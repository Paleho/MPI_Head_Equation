...
int main(int argc, char ** argv) {
	...
	*/
    ...
    for (t=0;t<T;t++) {
        ...
        RedSOR(u_previous, u_current, i_min, i_max+1, j_min, j_max+1,omega);
        gettimeofday(&tcf,NULL);

        tcomp += (tcf.tv_sec - tcs.tv_sec) + (tcf.tv_usec - tcs.tv_usec) * 0.000001;

        for(i = 0; i < local[0]; i++){
            west_send_buffer[i] = u_current[i+1][1];

            east_send_buffer[i] = u_current[i+1][local[1]];
        }

        req_counter = 0;
        if(north != MPI_PROC_NULL) MPI_Isend(&(u_current[1][1]), local[1], MPI_DOUBLE, north, 0, MPI_COMM_WORLD, &request[req_counter++]);
        if(south != MPI_PROC_NULL) MPI_Isend(&(u_current[local[0]][1]), local[1], MPI_DOUBLE, south, 1, MPI_COMM_WORLD, &request[req_counter++]);
        if(west != MPI_PROC_NULL)  MPI_Isend(west_send_buffer, local[0], MPI_DOUBLE, west, 3, MPI_COMM_WORLD, &request[req_counter++]);
        if(east != MPI_PROC_NULL)  MPI_Isend(east_send_buffer, local[0], MPI_DOUBLE, east, 2, MPI_COMM_WORLD, &request[req_counter++]);


        if(north != MPI_PROC_NULL) MPI_Irecv(&(u_current[0][1]), local[1], MPI_DOUBLE, north, MPI_ANY_TAG, MPI_COMM_WORLD, &request[req_counter++]);
        if(south != MPI_PROC_NULL) MPI_Irecv(&(u_current[local[0]+1][1]), local[1], MPI_DOUBLE, south, MPI_ANY_TAG, MPI_COMM_WORLD, &request[req_counter++]);
        if(west != MPI_PROC_NULL)  MPI_Irecv(west_recv_buffer, local[0], MPI_DOUBLE, west, MPI_ANY_TAG, MPI_COMM_WORLD, &request[req_counter++]);
        if(east != MPI_PROC_NULL)  MPI_Irecv(east_recv_buffer, local[0], MPI_DOUBLE, east, MPI_ANY_TAG, MPI_COMM_WORLD, &request[req_counter++]);
        ...
        for(i = 0; i < local[0]; i++){
            u_current[i+1][0] = west_recv_buffer[i];

            u_current[i+1][local[1]+1] = east_recv_buffer[i];
        }

		/*Compute */
        gettimeofday(&tcs,NULL);
        BlackSOR(u_previous, u_current, i_min, i_max+1, j_min, j_max+1,omega);
        gettimeofday(&tcf,NULL);
    ...
    if (rank==0) {
        ...
        #ifdef TEST_CONV
        printf("RedBlackSOR X %d Y %d Px %d Py %d Iter %d ComputationTime %lf ConvergenceTime %lf TotalTime %lf midpoint %lf\n",global[0],global[1],grid[0],grid[1],t,comp_time,conv_time,total_time,U[global[0]/2][global[1]/2]);

        #ifdef PRINT_RESULTS
        char * s=malloc(50*sizeof(char));
        sprintf(s,"resRBSORMPI_conv_%dx%d_%dx%d",global[0],global[1],grid[0],grid[1]);
        ...
        #ifdef PRINT_RESULTS
        char * s=malloc(50*sizeof(char));
        sprintf(s,"resRBSORMPI_%dx%d_%dx%d",global[0],global[1],grid[0],grid[1]);
        fprint2d(s,U,global[0],global[1]);
        free(s);
        #endif
